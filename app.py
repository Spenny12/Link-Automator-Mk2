...
if run_button:
    ...
    if os.path.exists(faiss_path) and os.path.exists(chunk_data_path) and os.path.exists(url_text_path):
        ...
        if not valid_targets:
            st.error("Target URLs not found in the cached data. Please clear the cache and re-crawl, or ensure the targets were in the sitemap.")
            st.stop()
        ...
        with st.spinner("Generating semantic embeddings and saving to local cache..."):
            ...
            st.success(f"Successfully cached {len(chunk_data)} text chunks for {domain_name}!")

    # --- Step 3: Semantic Search ---
    st.info("Indexing Complete! Finding semantic matches and generating suggestions...")
    ...
    for target_url in valid_targets:
        ...
        distances, indices = faiss_index.search(target_vector, 15)
